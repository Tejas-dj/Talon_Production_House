...
- **Gate verification (one real image via Cloudinary, one real video via Bunny, checked on a physical phone) is blocked on real credentials and a physical device** — the loader, presets, player component, and `.env.example` are built and pass `next build`/lint with placeholder-shaped code paths, but the actual gate needs the client's real Cloudinary cloud name and Bunny pull-zone hostname (asked for in chat per the brief) plus a phone check I can't perform myself; verification completes once those are supplied.
